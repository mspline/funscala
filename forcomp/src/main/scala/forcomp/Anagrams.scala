...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(c => c.toLower).toList.map{case (c, s) => (c, s.length)}.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (c, len) :: os => {
      ...
      } yield if (l == 0) comb else (c, l) :: comb
    }.toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = (x, y) match {
    case (xs, Nil) => xs
    case (Nil, ys) => Nil
    case ((xc, xl) :: xt, (yc, yl) :: yt) => {
      if (xc == yc) {
        val l = xl - yl
        if (l <= 0) subtract(xt, yt) else (xc, l) :: subtract(xt, yt)
      }
      else if (xc < yc) {
        (xc, xl) :: subtract(xt, y)
      }
